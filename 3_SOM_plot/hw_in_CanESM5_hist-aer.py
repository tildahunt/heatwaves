...
daysperyear = 365
dayspersummer = 92
date_example = '18500101-20201231'
...
lsm = xr.open_dataset(f'/gws/nopw/j04/aopp/tildah/HW_metrics/lsms/sftlf_{model}.nc')
land_mask = lsm['sftlf']
land_mask_binary = xr.where(land_mask > 30, 1, 0)

input_pattern = f"/gws/nopw/j04/leader_epesc/incoming/day/tasmax_day/{exp}/{model}/tasmax_day_{model}_{exp}_*_gn_{date_example}.nc"
...
for file_path in file_list:
    ...
    print(unique_code)
    output_path = f"~/Internship/CIRCULATION/heatwave_in_*/{model}_{exp}/{model}_{exp}_{unique_code}.nc"
    if os.path.exists(output_path):
            print(f"Skipping {unique_code} as {output_path} already exists.")
            continue

    data = xr.open_mfdataset(f"/gws/nopw/j04/leader_epesc/incoming/day/tasmax_day/{exp}/{model}/tasmax_day_{model}_{exp}_{unique_code}_gn_*.nc", combine='by_coords', chunks={'time': daysperyear})
    #print(data)
    heatwaves = data_to_heatwaves(data)
    print(heatwaves)
    land_mask_binary = land_mask_binary.assign_coords(lat=heatwaves.lat, lon=heatwaves.lon)
    land_hws = heatwaves.where(land_mask_binary == 1)
    
    heatwave_in_europe = heatwave_in_region(land_hws, 36, 72)
    heatwave_in_north = heatwave_in_region(land_hws, 50, 72)
    heatwave_in_south = heatwave_in_region(land_hws, 36, 50)
    ...
    print(f"Saved {output_path} in {timer.time() - start_time:.2f} seconds")
